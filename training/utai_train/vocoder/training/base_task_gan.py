# vendored: openvpi/SingingVocoders @ 4d0889c (2026-03-08) -- training/base_task_gan.py
# 逐字拷贝;仅改动 = 本头注 + import 行重写到 utai_train.vocoder 命名空间(见 vendor_vocoder.py)
# + 两处登记偏离(设计红队 A2 同批):
#   (1) train/val dataloader 的 persistent_workers/prefetch_factor 按 num_workers==0
#       条件化——上游硬编码 persistent_workers=True 且恒传 prefetch_factor,torch 在
#       num_workers=0 时二者皆 ValueError(gate1 CPU 对拍与 CPU 冒烟需要 workers=0;
#       生产 ds_workers=4 路径行为不变;gate1 原版侧由 harness 以同款 monkeypatch 对齐)。
#   (2) load_pre_train_model 的 torch.load 加 map_location="cpu"——CUDA 存档的社区
#       底模在 CPU 训练(force_cpu 兜底)下上游直接 RuntimeError;数值中性。
# 其余数学/RNG/控制流与上游逐字一致;新行为一律在 harness.py/pipeline.py 实现,不改本文件。
import logging
import os
import pathlib
import sys
...
import utai_train.vocoder.utils as utils
# from utils.indexed_datasets import IndexedDataset
from utai_train.vocoder.utils.training_utils import (
...
class GanBaseTask(pl.LightningModule):
    ...
    def build_scheduler(self, optimizer):
        from utai_train.vocoder.utils import build_lr_scheduler_from_config
    ...
    def build_optimizer(self, model, optimizer_args):
        from utai_train.vocoder.utils import build_object_from_class_name
    ...
    def train_dataloader(self):
        ...
        # )
        # 登记偏离(A2,见头注): workers==0 时 persistent/prefetch 必须归零
        _nw = self.config['ds_workers']
        return torch.utils.data.DataLoader(self.train_dataset,
                                           collate_fn=self.train_dataset.collater,
                                           batch_size=self.config['batch_size'],
                                           # batch_sampler=self.training_sampler,
                                           num_workers=_nw,
                                           prefetch_factor=self.config['dataloader_prefetch_factor'] if _nw > 0 else None,
                                           pin_memory=True,
    ...
    def val_dataloader(self):
        ...
        #     batch_by_size=False
        # )
        # 登记偏离(A2,见头注): workers==0 时 prefetch 必须归零
        _nw = self.config['ds_workers']
