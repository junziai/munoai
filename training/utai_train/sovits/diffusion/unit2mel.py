# Vendored from so-vits-svc 4.1-Stable diffusion/unit2mel.py (@ 730930d).
# Model math UNCHANGED (unit/f0/volume embeds, aug_shift/5, GaussianDiffusion
# decoder, k_step_max resolution :87). Deviations: print() -> logging only
# (load_model_vocoder is the inference-side loader, unused by training, kept
# for completeness).
import logging
import os
...
def load_model_vocoder(
    ...
    model.eval()
    logger.info('Loaded diffusion model, sampler is %s, speedup: %s', args.infer.method, args.infer.speedup)
