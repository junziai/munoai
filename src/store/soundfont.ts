...
/** Rust `SoundfontPresetEntry`(snake→camel 在命令层手工映射)。 */
export interface SoundfontPreset {
  id: string;
  name: string;
}

/** Rust `SoundfontEntry`。format: "sfz" | "sf2"。 */
...
/** 推荐音源(工程规划第四部分):纯链接表,不含任何文件——软件只展示、点击后开系统浏览器。
 *  license/size 仅展示参考。 */
export interface RecommendedSoundfont {
  /** 用途(轨道类型关键字)。 */
  purposeKey: string;
  name: string;
  format: "sfz" | "sf2" | "sfz/sf2";
  size: string;
  url: string;
  license: string;
  /** 自动音色映射的关键字(小写子串匹配 fontId)——与 Rust/ArrangementTrack 分配规则一致。 */
  matchKeywords: string[];
}

export const RECOMMENDED_SOUNDFONTS: RecommendedSoundfont[] = [
  {
...
export type ArrangeTrackKind =
  | "drums" | "bass" | "piano" | "chords" | "lead"
 ...
 */
export const GM_PROGRAM: Record<ArrangeTrackKind, number> = {
  drums: 0,
...
export function defaultSoundfontFor(
  trackKind: ArrangeTrackKind,
  fonts: Soundfont[],
): Soundfont | null {
  const keywordByKind: Record<ArrangeTrackKind, string[]> = {
...
export function firstPresetOf(font: Soundfont): SoundfontPreset | null {
...
export function presetForGmProgram(font: Soundfont, program: number): SoundfontPreset | null {
  if (font.format === "sf2") {
...
export interface PickedInstrumentSoundfont {
  font: Soundfont;
...
interface SoundfontState {
  ...
  importFont: (path: string) => Promise<Soundfont | null>;
  removeFont: (id: string) => Promise<boolean>;
}

function toFont(raw: Record<string, unknown>): Soundfont {
...
export const useSoundfontStore = create<SoundfontState>((set, get) => ({
  fonts: [],
  scanning: false,
  importingPaths: [],

  refresh: async () => {
    if (!isTauri()) return; // No Rust backend in browser preview — silently skip
    if (get().scanning) return;
    set({ scanning: true });
    try {
      const raw = await invoke<Record<string, unknown>[]>("list_soundfonts");
  ...
  importFont: async (path) => {
    if (get().importingPaths.includes(path)) return null;
    set((s) => ({ importingPaths: [...s.importingPaths, path] }));
    try {
      const raw = await invoke<Record<string, unknown>>("import_soundfont", { path });
