...
import type { ProcessedOutput, Segment, Track, TrackSoundfont } from "../../types/project";
...
/** wire 上的渲染音符(Rust `RenderNote` 镜像;snake-free:字段名与 Rust 一致)。 */
export interface InstrumentRenderNote {
  start: number;
  dur: number;
  key: number;
  vel: number;
...
export function isInstrumentDirty(track: Track, seg: Segment, tempo: number): boolean {
  if (track.trackType !== "instrument" || seg.content.type !== "notes") return false;
  const bake = seg.processedOutputs?.find((o) => o.laneId === INSTRUMENT_LANE_ID && !o.loading);
  if (seg.content.notes.length === 0) return !!bake;
  if (!track.soundfont) return false;
  if (!bake) return true;
  return bake.renderedSig !== instrumentRenderSig(track, seg, tempo);
}

/** 全工程脏乐器段落(实时读)。空 ⇒ Play 零附加延迟。 */
export function collectDirtyInstruments(tempo: number): Array<{ trackId: string; segmentId: string }> {
  const out: Array<{ trackId: string; segmentId: string }> = [];
  for (const tr of useProjectStore.getState().tracks) {
    if (tr.trackType !== "instrument") continue;
    for (const sg of tr.segments) {
      if (isInstrumentDirty(tr, sg, tempo)) out.push({ trackId: tr.id, segmentId: sg.id });
    }
  }
  return out;
}
...
export function instrumentTracksMissingFont(): Track[] {
  return useProjectStore
    .getState()
    .tracks.filter(
      (tr) =>
        tr.trackType === "instrument" &&
        !tr.soundfont &&
...
export async function renderInstrumentPart(
  track: Track,
  seg: Segment,
  tempo: number,
): Promise<void> {
  ...
  const sf = track.soundfont;
  if (!sf) throw new Error(i18n.t("soundfont.trackNoneTip"));

  const segRef = () =>
    useProjectStore.getState().tracks.find((t) => t.id === track.id)?.segments.find((s) => s.id === seg.id);
  const prevOutputs = segRef()?.processedOutputs; // 现有烘焙保留;失败时还原
  const deposit = (outs: ProcessedOutput[] | undefined) =>
    useProjectStore.getState().replaceProcessedOutputs(track.id, seg.id, outs ?? []);
  ...
  try {
    const notes: InstrumentRenderNote[] = seg.content.notes.map((n) => ({
      start: tickToSec(n.tick, tempo),
      dur: tickToSec(n.duration, tempo),
      key: n.pitch,
      vel: n.velocity ?? 100,
    }));
    const wavPath = await invoke<string>("render_soundfont_notes", {
      fontId: sf.fontId,
    ...
    if (segRef()) deposit(prevOutputs);
    throw e;
  }
}

export interface InstrumentDirtyResult {
  rendered: number;
  failed: number;
  cancelled: boolean;
}
...
export async function renderDirtyInstruments(
  list: Array<{ trackId: string; segmentId: string }>,
  tempo: number,
  opts?: { shouldCancel?: () => boolean },
): Promise<InstrumentDirtyResult> {
  let rendered = 0;
  let failed = 0;
  ...
  for (const { trackId, segmentId } of list) {
    ...
    if (!tr || !sg || !isInstrumentDirty(tr, sg, tempo)) continue;
...
export async function playSoundfontAudition(sf: TrackSoundfont, key: number, vel?: number): Promise<void> {
  const seq = ++auditionSeq;
  try {
    // Muno 阶段2:优先走 Rust cpal 常驻实时流(乐器已缓存时零延迟)——命令返回
    // null 表示已直接发声,前端无事可做;返回路径 = 实时不可用,回退 WAV+WebAudio。
    const wavPath = await invoke<string | null>("audition_soundfont_note", {
      fontId: sf.fontId,
      presetId: sf.presetId,
    ...
    if (seq !== auditionSeq) return; // 已被更新的试听取代 → 丢弃
    if (!wavPath) return; // 实时引擎已直接发声
